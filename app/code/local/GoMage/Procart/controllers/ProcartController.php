...
class GoMage_Procart_ProcartController extends Mage_Core_Controller_Front_Action{
    ...
    public function changeqtycartitemAction() {
	    ...
	    if (($result['qty'] = $this->getRequest()->getParam('qty')) &&
	        ...
	        try {
                ...
                $incrementsCheck = $product->getStockItem()->checkQtyIncrements($result['qty']);
                if($incrementsCheck->getHasError()){
                    $result['error'] = true;
                    $result['message'] = $incrementsCheck->getMessage();
                }
            ...
            if($this->getRequest()->getParam('cart') == 1){
                ...
                foreach($items as $cartItem){
                    ...
                    if($cartItem->getHasError()){
                        $result['error'] = true;
                        $result['message'] = $cartItem->getMessage();
                    }
                }
            ...
            if (!$result['error'] && $this->getRequest()->getParam('cart') == 1){
                ...
                $blocksHelper = Mage::helper('gomage_procart/blocks');
                if($checkoutMethods_top = $blocksHelper->getCartCheckoutMethods('top_methods')){
                    $result['checkout_methods_top'] = $checkoutMethods_top;
                }
                if($checkoutMethods = $blocksHelper->getCartCheckoutMethods('methods')){
                    $result['checkout_methods_bottom'] = $checkoutMethods;
                }
            }
	...
	{
	    $items_html = '';
        /* @var $blocksHelper GoMage_Procart_Helper_Blocks */
	    $blocksHelper = Mage::helper('gomage_procart/blocks');
        $cart = $blocksHelper->getShoppingCartBlock();
                 ...
                 if($product->isComposite()){
                     ...
                     foreach($ti->getChildrenIds($product->getId()) as $groupIds){
                         foreach($groupIds as $id){
                             ...
                             $product_list[$childProduct->getId()]
