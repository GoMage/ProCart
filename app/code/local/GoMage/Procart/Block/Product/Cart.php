...
class GoMage_Procart_Block_Product_Cart extends Mage_Core_Block_Template
{
    ...
    public function getProcartProductList()
    {
        ...
        if (!$this->_procartproductlist) {
            ...
            foreach ($cartItems as $item) {
                ...
                if ($product = Mage::getModel('catalog/product')->load($item->getProduct()->getId())) {

                    $crosssell = $product->getCrossSellProducts();

                    foreach ($crosssell as $_product) {
