...
class GoMage_Procart_Block_Config extends Mage_Core_Block_Template
{
    ...
    public function getConfig()
    {
        if (!$this->_config) {
            ...
            if ($helper->getIsUltimentoTheme()) {
                $this->_config['slide_destination'] = '#customer-nav-cart';
            } else {
            ...
            $this->_config['background_view']   = Mage::getStoreConfig('gomage_procart/confirm_wind/background_view');
            $this->_config['window_width']      = Mage::getStoreConfig('gomage_procart/confirm_wind/width');
            ...
            $this->_config['movetowishlist_url']  = $this->getUrl('gomageprocart/procart/movetowishlist');
            $this->_config['ultimento_theme']     = $helper->getIsUltimentoTheme() ? 1 : 0;
            $this->_config['product_review_page'] = $helper->isProductReviewPage() ? 1 : 0;
    ...
    public function getQtyUpsellTemplate()
    {
        $template = Mage::helper('gomage_procart/qty')
            ->getQtyTemplate(GoMage_Procart_Helper_Qty::QTY_TEMPLATE_UPSEL);
    ...
    {
        return Mage::getStoreConfig('gomage_procart/confirm_window/continue_btn_txt');
    }

    public function getCartButtonText()
    {
        return Mage::getStoreConfig('gomage_procart/confirm_window/cart_btn_txt');
    }

    public function getCheckoutButtonText()
    {
        return Mage::getStoreConfig('gomage_procart/confirm_window/checkout_btn_txt');
    }

    public function getWishlistButtonText()
    {
        return Mage::getStoreConfig('gomage_procart/confirm_window/wishlist_btn_txt');
    }

    public function getCompareButtonText()
    {
        return Mage::getStoreConfig('gomage_procart/confirm_window/compare_btn_txt');
    }
}